...
def bev_settings(input_args=sys.argv[1:]):
    ...
    if not os.path.exists(args.smpl_path):
        smpl_url = 'https://github.com/ZaiqiangWu/ROMP/releases/download/V2.0/smpla_packed_info.pth'
        #https://github.com/ZaiqiangWu/ROMP/releases/download/V2.0/BEV.pth
        download_model(smpl_url, args.smpl_path, 'SMPL-A')
    if not os.path.exists(args.smil_path):
        smil_url = 'https://github.com/ZaiqiangWu/ROMP/releases/download/V2.0/smil_packed_info.pth'
        download_model(smil_url, args.smil_path, 'SMIL')
    if not os.path.exists(args.model_path):
        romp_url = 'https://github.com/ZaiqiangWu/ROMP/releases/download/V2.0/'+model_dict[model_id]
...
class BEV(nn.Module):
    ...
    def _build_model_(self):
        ...
        model.load_state_dict(torch.load(self.settings.model_path, map_location=self.tdevice,weights_only=True), strict=False)
